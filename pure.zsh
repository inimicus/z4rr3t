...
prompt_z4rr3t_preprompt_render() {
	...
	local git_dirty_color=$prompt_z4rr3t_colors[git:dirty]
	...
	if [[ -n $prompt_z4rr3t_vcs_info[branch] ]]; then
		local branch="%F{$git_color}"'${prompt_z4rr3t_vcs_info[branch]}'
		if [[ -n $prompt_z4rr3t_vcs_info[action] ]]; then
			branch+="|%F{$prompt_z4rr3t_colors[git:action]}"'$prompt_z4rr3t_vcs_info[action]'"%F{$git_color}"
		fi
		preprompt_parts+=("$branch""%F{$git_dirty_color}"'${prompt_z4rr3t_git_dirty}%f')
	fi
	# Git pull/push arrows.
	if [[ -n $prompt_g4rr3t_git_arrows ]]; then
		preprompt_parts+=('%F{$prompt_g4rr3t_colors[git:arrow]}${prompt_g4rr3t_git_arrows}%f')
	fi

	# Username and machine, if applicable.
	[[ -n $prompt_g4rr3t_state[username] ]] && preprompt_parts+=($prompt_g4rr3t_state[username])
	# Execution time.
	[[ -n $prompt_g4rr3t_cmd_exec_time ]] && preprompt_parts+=('%F{$prompt_g4rr3t_colors[execution_time]}${prompt_g4rr3t_cmd_exec_time}%f')
	...
	if [[ $1 == precmd ]]; then
		...
		print
	elif [[ $prompt_g4rr3t_last_prompt != $expanded_prompt ]]; then
		# Redraw the prompt.
		prompt_g4rr3t_reset_prompt
	fi

	typeset -g prompt_g4rr3t_last_prompt=$expanded_prompt
}

prompt_g4rr3t_precmd() {
	# Check execution time and store it in a variable.
	prompt_g4rr3t_check_cmd_exec_time
	unset prompt_g4rr3t_cmd_timestamp

	# Shows the full path in the title.
	prompt_g4rr3t_set_title 'expand-prompt' '%~'

	# Modify the colors if some have changed..
	prompt_g4rr3t_set_colors

	# Perform async Git dirty check and fetch.
	prompt_g4rr3t_async_tasks
	...
	prompt_g4rr3t_reset_prompt_symbol

	# Print the preprompt.
	prompt_g4rr3t_preprompt_render "precmd"
...
prompt_g4rr3t_async_git_aliases() {
	setopt localoptions noshwordsplit
...
prompt_g4rr3t_async_git_fetch() {
	...
	prompt_g4rr3t_async_git_arrows
}
...
prompt_g4rr3t_async_tasks() {
	...
	((!${prompt_g4rr3t_async_init:-0})) && {
		async_start_worker "prompt_pure" -u -n
		async_register_callback "prompt_pure" prompt_g4rr3t_async_callback
		typeset -g prompt_g4rr3t_async_init=1
	}

	# Update the current working directory of the async worker.
	async_worker_eval "prompt_pure" builtin cd -q $PWD

	typeset -gA prompt_g4rr3t_vcs_info

	local -H MATCH MBEGIN MEND
	if [[ $PWD != ${prompt_g4rr3t_vcs_info[pwd]}* ]]; then
		# Stop any running async jobs.
		async_flush_jobs "prompt_pure"

		# Reset Git preprompt variables, switching working tree.
		unset prompt_g4rr3t_git_dirty
		unset prompt_g4rr3t_git_last_dirty_check_timestamp
		unset prompt_g4rr3t_git_arrows
		unset prompt_g4rr3t_git_fetch_pattern
		prompt_g4rr3t_vcs_info[branch]=
		prompt_g4rr3t_vcs_info[top]=
	fi
	unset MATCH MBEGIN MEND

	async_job "prompt_pure" prompt_g4rr3t_async_vcs_info

	# Only perform tasks inside a Git working tree.
	[[ -n $prompt_g4rr3t_vcs_info[top] ]] || return

	prompt_g4rr3t_async_refresh
}

prompt_g4rr3t_async_refresh() {
	setopt localoptions noshwordsplit

	if [[ -z $prompt_g4rr3t_git_fetch_pattern ]]; then
		# We set the pattern here to avoid redoing the pattern check until the
		# working three has changed. Pull and fetch are always valid patterns.
		typeset -g prompt_g4rr3t_git_fetch_pattern="pull|fetch"
		async_job "prompt_pure" prompt_g4rr3t_async_git_aliases
	fi

	async_job "prompt_pure" prompt_g4rr3t_async_git_arrows

	# Do not preform `git fetch` if it is disabled or in home folder.
	if (( ${Z4RR3T_GIT_PULL:-1} )) && [[ $prompt_g4rr3t_vcs_info[top] != $HOME ]]; then
		# Tell the async worker to do a `git fetch`.
		async_job "prompt_pure" prompt_g4rr3t_async_git_fetch
	fi

	# If dirty checking is sufficiently fast,
	# tell the worker to check it again, or wait for timeout.
	integer time_since_last_dirty_check=$(( EPOCHSECONDS - ${prompt_g4rr3t_git_last_dirty_check_timestamp:-0} ))
	if (( time_since_last_dirty_check > ${Z4RR3T_GIT_DELAY_DIRTY_CHECK:-1800} )); then
		unset prompt_g4rr3t_git_last_dirty_check_timestamp
		# Check check if there is anything to pull.
		async_job "prompt_pure" prompt_g4rr3t_async_git_dirty ${Z4RR3T_GIT_UNTRACKED_DIRTY:-1}
	fi
}

prompt_g4rr3t_check_git_arrows() {
	setopt localoptions noshwordsplit
...
prompt_g4rr3t_async_callback() {
	...
	case $job in
		\[async])
			...
			if [[ $code -eq 2 ]]; then
				...
				typeset -g prompt_g4rr3t_async_init=0
			fi
			;;
		prompt_g4rr3t_async_vcs_info)
			local -A info
			...
			if [[ $info[top] = $prompt_g4rr3t_vcs_info[top] ]]; then
				# If the stored pwd is part of $PWD, $PWD is shorter and likelier
				# to be top-level, so we update pwd.
				if [[ $prompt_g4rr3t_vcs_info[pwd] = ${PWD}* ]]; then
					prompt_g4rr3t_vcs_info[pwd]=$PWD
				fi
			else
			...
			[[ -n $info[top] ]] && [[ -z $prompt_g4rr3t_vcs_info[top] ]] && prompt_g4rr3t_async_refresh

			# Always update branch and top-level.
			prompt_g4rr3t_vcs_info[branch]=$info[branch]
			prompt_g4rr3t_vcs_info[top]=$info[top]
			prompt_g4rr3t_vcs_info[action]=$info[action]
		...
		prompt_g4rr3t_async_git_aliases)
			if [[ -n $output ]]; then
				...
				prompt_g4rr3t_git_fetch_pattern+="|$output"
			fi
			;;
		prompt_g4rr3t_async_git_dirty)
			local prev_dirty=$prompt_g4rr3t_git_dirty
			if (( code == 0 )); then
				unset prompt_g4rr3t_git_dirty
			else
				typeset -g prompt_g4rr3t_git_dirty="*"
			fi

			[[ $prev_dirty != $prompt_g4rr3t_git_dirty ]] && do_render=1

			# When `prompt_g4rr3t_git_last_dirty_check_timestamp` is set, the Git info is displayed
			...
			(( $exec_time > 5 )) && prompt_g4rr3t_git_last_dirty_check_timestamp=$EPOCHSECONDS
			;;
		prompt_g4rr3t_async_git_fetch|prompt_g4rr3t_async_git_arrows)
			# `prompt_g4rr3t_async_git_fetch` executes `prompt_g4rr3t_async_git_arrows`
			# after a successful fetch.
			case $code in
				0)
					local REPLY
					prompt_g4rr3t_check_git_arrows ${(ps:\t:)output}
					if [[ $prompt_g4rr3t_git_arrows != $REPLY ]]; then
						typeset -g prompt_g4rr3t_git_arrows=$REPLY
						do_render=1
					fi
					;;
				99|98)
					...
					;;
				*)
					# Non-zero exit status from `prompt_g4rr3t_async_git_arrows`,
	...
	if (( next_pending )); then
		(( do_render )) && typeset -g prompt_g4rr3t_async_render_requested=1
		return
	fi

	[[ ${prompt_g4rr3t_async_render_requested:-$do_render} = 1 ]] && prompt_g4rr3t_preprompt_render
	unset prompt_g4rr3t_async_render_requested
}
...
prompt_g4rr3t_reset_prompt_symbol() {
	prompt_g4rr3t_state[prompt]=${Z4RR3T_PROMPT_SYMBOL:-❯}
}

prompt_g4rr3t_update_vim_prompt_widget() {
	setopt localoptions noshwordsplit
	prompt_g4rr3t_state[prompt]=${${KEYMAP/vicmd/${Z4RR3T_PROMPT_VICMD_SYMBOL:-❮}}/(main|viins)/${Z4RR3T_PROMPT_SYMBOL:-❯}}

	prompt_g4rr3t_reset_prompt
}

prompt_g4rr3t_reset_vim_prompt_widget() {
	setopt localoptions noshwordsplit
	prompt_g4rr3t_reset_prompt_symbol
...
prompt_g4rr3t_state_setup() {
	setopt localoptions noshwordsplit

	# Check SSH_CONNECTION and the current state.
	local ssh_connection=${SSH_CONNECTION:-$PROMPT_g4rr3t_SSH_CONNECTION}
	...
	if [[ -z $ssh_connection ]] && (( $+commands[who] )); then
		...
		if [[ $who_out =~ "\(?($reIPv4|$reIPv6|$reHostname)\)?\$" ]]; then
			...
			export PROMPT_g4rr3t_SSH_CONNECTION=$ssh_connection
		fi
		unset MATCH MBEGIN MEND
	fi

	hostname='%F{$prompt_g4rr3t_colors[host]}@%m%f'
	# Show `username@host` if logged in through SSH.
	[[ -n $ssh_connection ]] && username='%F{$prompt_g4rr3t_colors[user]}%n%f'"$hostname"

	# Show `username@host` if root, with username in default color.
	[[ $UID -eq 0 ]] && username='%F{$prompt_g4rr3t_colors[user:root]}%n%f'"$hostname"

	typeset -gA prompt_g4rr3t_state
	prompt_g4rr3t_state[version]="1.10.3"
	prompt_g4rr3t_state+=(
		username "$username"
...
prompt_g4rr3t_system_report() {
	...
	print - "- z4rr3t state:"
	for k v in "${(@kv)prompt_g4rr3t_state}"; do
		print - "\t- $k: \`${(q)v}\`"
	done
...
prompt_g4rr3t_setup() {
	...
	# Set the colors.
	typeset -gA prompt_g4rr3t_colors_default prompt_g4rr3t_colors
	prompt_g4rr3t_colors_default=(
	...
	prompt_g4rr3t_colors=("${(@kv)prompt_g4rr3t_colors_default}")

	add-zsh-hook precmd prompt_g4rr3t_precmd
	add-zsh-hook preexec prompt_g4rr3t_preexec

	prompt_g4rr3t_state_setup

	zle -N prompt_g4rr3t_reset_prompt
	zle -N prompt_g4rr3t_update_vim_prompt_widget
	zle -N prompt_g4rr3t_reset_vim_prompt_widget
	if (( $+functions[add-zle-hook-widget] )); then
		add-zle-hook-widget zle-line-finish prompt_g4rr3t_reset_vim_prompt_widget
		add-zle-hook-widget zle-keymap-select prompt_g4rr3t_update_vim_prompt_widget
	fi

	# If a virtualenv is activated, display it in grey.
	PROMPT='%(12V.%F{$prompt_g4rr3t_colors[virtualenv]}%12v%f .)'

	# Prompt turns red if the previous command didn't exit with 0.
	PROMPT+='%(?.%F{$prompt_g4rr3t_colors[prompt:success]}.%F{$prompt_g4rr3t_colors[prompt:error]})${prompt_g4rr3t_state[prompt]}%f '

	# Indicate continuation prompt by … and use a darker color for it.
	PROMPT2='%F{242}%_… %f%(?.%F{magenta}.%F{red})${prompt_g4rr3t_state[prompt]}%f '

	# Store prompt expansion symbols for in-place expansion via (%). For
	# some reason it does not work without storing them in a variable first.
	typeset -ga prompt_g4rr3t_debug_depth
	prompt_g4rr3t_debug_depth=('%e' '%N' '%x')
	...
	local -A ps4_parts
	ps4_parts=(
		depth 	  '%F{yellow}${(l:${(%)prompt_g4rr3t_debug_depth[1]}::+:)}%f'
		compare   '${${(%)prompt_g4rr3t_debug_depth[2]}:#${(%)prompt_g4rr3t_debug_depth[3]}}'
		main      '%F{blue}${${(%)prompt_g4rr3t_debug_depth[3]}:t}%f%F{242}:%I%f %F{242}@%f%F{blue}%N%f%F{242}:%i%f'
